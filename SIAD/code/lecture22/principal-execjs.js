...
function createCORSRequest(method, url) {
  ...
  if ("withCredentials" in xhr) {
    ...
    xhr.open(method, "proxy?url="+url, true);
...
function executeJavaScript(principal,url) {
   	var xhttp = new XMLHttpRequest();
   	xhttp.onreadystatechange = function() {
	    if (this.readyState == 4 && this.status == 200) {
	      execScript(principal,xhttp.responseText);
	    }
  	};
...
loadLocalJS=function(principal,url){
    // alert("load executev" + url + " under principal " + principal);
    executeJavaScript(principal,url);
}
})();
